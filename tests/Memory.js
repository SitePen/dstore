define([
	...
	registerSuite({
		...
		'total property': function(){
			...
			assert.strictEqual(filteredCollection.total, filteredCollection.store.data.length);

			var sortedCollection = store.sort("id");
			assert.strictEqual(sortedCollection.total, sortedCollection.store.data.length);

			var rangedCollection = store.range(0, 5);
			assert.strictEqual(rangedCollection.total, rangedCollection.store.data.length);
		}
	});
