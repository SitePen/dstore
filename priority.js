/*
This is a store wrapper that provides prioritized operations. One can include a "priority" property in the options property that is a number representing the priority, for
get, put, add, remove, or query calls. The associated action will execute before any other actions in the queue that have a lower priority. The priority queue will also
throttle the execution of actions to limit the number of concurrent actions to the priority number. If you submit an action with a priority of 3, it will not be executed until there
are at most 2 other concurrent actions.
*/
				...
				priorityStore[method] = function(first, options){
					...
					if(method == 'query'){
						...
						executor = function(callback){
							...
							when(original.apply(store, args), function(value){
								def.resolve(value);
								callback(); // done
							},
							function(error){
					...
					// add to the queue
					var priority = options.priority || config.priority || 4;
