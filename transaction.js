/*
This is a transaction managing store. When transaction is started, by calling store.transaction(), all request operations (put, add, remove),
are entered into a transaction log. When commit() is called, the actions in the log are then committed.
This store relies on three other stores, extending the dojo/store/Cache:
...
define(['dojo/store/Memory', 'dojo/store/Cache', 'dojo/when', 'dojo/aspect', 'dojo/_base/lang'], 
	...
	return function(masterStore, cachingStore, options){
		...
		return Cache(lang.delegate(masterStore, {
			put: addToLog('put'),
			add: addToLog('add'),
			...
			commit: function(){
				...
				return transactionLogStore.query({}).map(function(action){
					...
					try{
						...
						var result = store[method](target, action.options);
					}catch(e){
						var result = transactionStore.errorHandler(e);
						if(result === true){
